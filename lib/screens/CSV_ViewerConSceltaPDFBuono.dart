...
//Inizio trattamento CSV con INTESTAZIONE
  ...
  Map<String, int> _createColumnIndexMap(List<String> headers) {
    ...
    for (int i = 0; i < headers.length; i++) {
      ...
      // La tua keyStrumento è 'Strum'. Se nel CSV l'header è 'Strumento',
            ...
            actions: <Widget>[
              ...
              ElevatedButton( // Ho cambiato in ElevatedButton per distinguerlo, ma puoi usare TextButton
                ...
                onPressed: () async  {
                  ...
                  /* if (kIsWeb) {
// Siamo su WEB
                   if (SelNumPag.isEmpty) {
                    if (dialogContext.mounted) {
                        ScaffoldMessenger.of(dialogContext).showSnackBar(
                        const SnackBar(content: Text('Il numero di pagina è richiesto per l\'apertura web.')),
                      );
                    }
                    return;
                    }
                    print('WEB: Tentativo di aprire "$percorsoPdfDaAprire" a pagina "$SelNumPag" nel browser esterno.');
                      await _openPdfInExternalBrowser(percorsoPdfDaAprire, SelNumPag);
                  } else {/// */
...
////VERSIONE Versione 2: Con SingleChildScrollView che avvolge l'intera Row per la gestione della CARD
          return Card(
            ...
            child: Padding(
              ...
              child: SingleChildScrollView( // Avvolge l'intera Row
                scrollDirection: Axis.horizontal, // Abilita lo scrolling orizzontale
                child: Row(
                  children: [
                    RichText( // Non è più avvolto da Expanded, prenderà la sua larghezza naturale
// overflow: TextOverflow.ellipsis, // Meno cruciale qui se lo scrolling gestisce la larghezza
// ma può essere lasciato se vuoi ellipsis se il testo è multilinea
// e vuoi comunque limitare l'altezza.
// maxLines: 1,                   // Utile se vuoi comunque limitare l'altezza a una riga
// anche se è scrollabile in larghezza.
// Se commentato, il testo può andare a capo e aumentare l'altezza della riga.
                      text: TextSpan(
                        text: "Trasp: $strumento ",
                        style: TextStyle(
                          fontSize: 11,
                          color: Colors.black,
                        ),
...
// Bottone per l'azione
          ...
          /// //// INIZIO Vecchia versione CARD con Dati Colorati
          /*
          return Card(
            // --- MODIFICHE PER ALTEZZA E COLORE ---
              margin: const EdgeInsets.symmetric(
                  horizontal: 8.0, vertical: 1.0), // Margine verticale ridotto (es. da 3.0 a 1.0)
              color: rowBackgroundColor, // Applica il colore di sfondo scelto
              elevation: 1.0, // Ombra leggermente ridotta per un look più pulito (opzionale)
              child: Padding(     //Una riga vuota
                padding: const EdgeInsets.symmetric(
                horizontal: 10.0, vertical: 2.0), // Padding verticale ridotto (es. da 6.0 a 5.0)
// e orizzontale leggermente aumentato per compensare
            //child: Padding(
            //  padding: const EdgeInsets.all(6.0),
              child: Row(
                children: [
                  //&&&&&&&&&& COPIATO DAGLI ESEMPI DI TextSpan

              //Strum
              //if (strumento.isNotEmpty
              //TextSpan(text: 'StrumTraspo:', style: TextStyle(fontWeight: FontWeight.w500, color: coloreDettagliSecondari)),
              ...
              RichText(
                  ...
                  text: TextSpan(  text: "Trasp: $strumento ",
                    style: TextStyle( // Stile di default per questo RichText
                    fontSize: 11,
                    color: Colors.black, // Colore di fallback, non dovrebbe essere usato se tutti i children hanno stile
                  ),
                  children: <TextSpan>[ // Lista di TextSpan figli
              //TextSpan(  //Lista dei figli per ciascuno un TextSpan COME ERA NELL'ESEMPIO
              ...
              // Mat TipoMulti
                 ...
                 // if (strumento.isNotEmpty
                  ...
                  //  ),
                      //numPag
                      //TextSpan(text: numPag.isNotEmpty ? 'A Pag: $numPag ' : "", style: TextStyle(fontWeight: FontWeight.normal, color: coloreDettagliPrimari)),
...
//*/
                  ...
                  // Bottone per l'azione
                  if (titolo != 'N/D' && volume != 'N/D') // Esempio: mostra se titolo e volume sono validi
                    IconButton(
                      icon: const Icon(Icons.picture_as_pdf_outlined, color: Colors.redAccent),
                      tooltip: 'Apri PDF',
                      onPressed: () {
...
//// FINE Vecchia versione CARD con Dati Colorati
*/
           */
        },
      ),
